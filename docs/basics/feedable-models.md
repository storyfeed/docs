...
| `feedMedia()` | read time, statically, from the cached snapshot | a fresh URL, and anything else that expires |

A feed page is served from snapshots. URLs are minted live so they never go
stale. The one call that reaches your tables is [`$context->model()`](#the-live-model).

`InteractsWithFeed` supplies a `feedMedia()` that returns null, so a model with
no page of its own writes only `toFeed()`. The entity renders at full weight,
unlinked. `toFeed()` has no default.

A resolver runs for every entity on a page, including the group exemplars a
renderer never draws as links, so it is a pure function of the context: no
writes, and no query except `$context->model()`. Throwing inside it is safe: the
failure is reported and the entity degrades to `url: null`. One broken resolver
never breaks a feed.
